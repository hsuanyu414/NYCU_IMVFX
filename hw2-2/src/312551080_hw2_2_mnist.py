...
# Create the mnist_log folder if not exist
log_dir = f"{workspace_dir}/mnist_log"
...
# Show images
def show_images(images, title="", save=False, filename="images.jpg"):
...
class DDPM(nn.Module):
    def __init__(self, image_shape=(3, 28, 28), n_steps=200, start_beta=1e-4, end_beta=0.02, device=None):
...
"""
def generate_new_images(ddpm, n_samples=16, device=None, frames_per_gif=25, gif_name="sampling_mnist.gif", channel=1, height=28, width=28):
...
def trainer(ddpm, dataloader, n_epochs, optim, loss_funciton, device, model_store_path):
    ...
    for epoch in tqdm(range(n_epochs), desc=f"Training progress", colour="green"):
        ...
        show_images(generate_new_images(ddpm, device=device), f"Images generated at epoch {epoch + 1}", save=True, filename=f"./mnist_log/epoch_{epoch + 1}.jpg")
...
######################################################################################
# save the result to "result_mnist.jpg"
nums_samples = 25
samples = generate_new_images(ddpm_mnist, n_samples=nums_samples, device=device)
show_images(samples, title="Final result", save=True, filename="result_mnist.jpg")
